...
public class DataCleanManager {
     ...
     * @param context
     */
    public static void deleteWebViewData(Context context) {
        File appCacheDir = new File(context.getFilesDir().getAbsolutePath() + "/webcache");
        Log.e("AppContext", "appCacheDir path=" + appCacheDir.getAbsolutePath());
        File webviewCacheDir = new File(context.getCacheDir().getAbsolutePath() + "/webviewCache");
        Log.e("AppContext", "webviewCacheDir path=" + webviewCacheDir.getAbsolutePath());
        //删除webview 缓存目录
        if (appCacheDir != null && appCacheDir.exists() && appCacheDir.isDirectory()) {
            for (File item : appCacheDir.listFiles()) {
                item.delete();
            }
            appCacheDir.delete();
        }
        ...
        if (webviewCacheDir != null && webviewCacheDir.exists() && webviewCacheDir.isDirectory()) {
            for (File item : webviewCacheDir.listFiles()) {
                item.delete();
            }
